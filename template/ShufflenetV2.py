...
possibilities = {'0.75': 96, '0.5': 48, '1.0': 116, '1.5': 176, '2.0': 224}
outchannel_dict = {'0.75': 1024, '0.5': 1024, '1.0': 1024, '1.5': 1024, '2.0': 2048}

def shufflenet_v2(images, is_training, num_classes=1000, depth_multiplier='1.0'):
    """
    ...
    """
    initial_depth = possibilities[str(depth_multiplier)]
    out_channels = outchannel_dict[str(depth_multiplier)]

    MOBILENETV1_CONV_DEFS = [
        Conv(kernel=[3, 3], stride=2, depth=24),
        DepthSepConv(kernel=[3, 3], stride=2, depth=initial_depth),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth),   #4 units
        DepthSepConv(kernel=[3, 3], stride=2, depth=initial_depth*2),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth*2),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth*2),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth*2),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth*2),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth*2),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth*2),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth*2),  #8 units
        DepthSepConv(kernel=[3, 3], stride=2, depth=initial_depth*4),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth*4),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth*4),
        DepthSepConv(kernel=[3, 3], stride=1, depth=initial_depth*4),  #4 units
        Conv(kernel=[1, 1], stride=1, depth=out_channels),
    ]
    depth = lambda d: d
    end_points = {}
    ...
    with tf.variable_scope('ShuffleNetV2'):
        ...
        with slim.arg_scope([slim.conv2d, depthwise_conv, slim.conv2d_transpose], **params):
            y = x
            for i, conv_def in enumerate(MOBILENETV1_CONV_DEFS):
                ...
                if isinstance(conv_def, Conv):
                    ...
                    if conv_def.stride == 2:
                        ...
                        with tf.variable_scope('unit_%d' % i):
                            x, y = basic_unit_with_downsampling(x, conv_def.depth)
                            end_points[end_point] = x
                    else:
                        end_point = end_point_base + '_basic_unit'
                        with tf.variable_scope('unit_%d' % i):
